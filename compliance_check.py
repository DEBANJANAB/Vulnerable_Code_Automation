...
def get_file_list_recursive(url, file_list = []):
    """
    Recursively retrieves all file URLs from a GitHub repository directory.

    Args:
        url (str): The API URL of the GitHub repository directory.
        file_list (list): A list to store the file URLs.
...
def get_first_level_files(url):
    ...
    """
    Retrieves the URLs of all files in the top level of a GitHub repository directory.
...
def download_files(repo_url, local_dir):
    ...
    """
    Downloads Python files from a GitHub repository to a local directory.
...
def run_bandit_on_file(b_mgr, file_path):
    """
    Runs Bandit security analysis on a single Python file.
    ...
    Returns:
        list: A list of issues found in the file.
    """
    try:
...
def format_issue(issue):
    """
    Formats a Bandit issue into a dictionary for CSV output.
...
def save_compliance_report(issues, output_file):
    ...
    unique_issues = set(issues)  # Convert list to set to remove duplicates
